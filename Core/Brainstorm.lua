...
function Brainstorm.init()
  ...
  elseif Brainstorm.debug.enabled then
    -- Fallback: force-create a simple log file if the structured logger isn't available
    local ok, fh = pcall(io.open, Brainstorm.PATH .. "/brainstorm.log", "a")
    if ok and fh then
      fh:write(string.format("[INFO] Brainstorm initialized (fallback logger) %s\n", Brainstorm.VERSION))
      fh:close()
    end
  end
