...
local function log_lua(message)
  -- Logging disabled.
  -- Re-enable only with a concrete debugging need; native logging is also a no-op.
end
  ...
  if not file_exists(config_path) then
    ...
    if success and loaded_config then
      ...
      Brainstorm.config.ar_prefs.suit_ratio_percent = Brainstorm.config.ar_prefs.suit_ratio_percent
        or "Disabled"
      if Brainstorm.config.ar_prefs.use_cuda == nil then
        Brainstorm.config.ar_prefs.use_cuda = true
      end

      -- Map suit ratio percentage to decimal value (use static table)
      local ratio_percent = Brainstorm.config.ar_prefs.suit_ratio_percent
      Brainstorm.config.ar_prefs.suit_ratio_decimal = Brainstorm.RATIO_MAP[ratio_percent]
        or 0
    end
  end
end
...
local function init_ffi()
  if not ffi_loaded then
    local success = pcall(
      ...
      // Enable or disable CUDA acceleration from the UI preference.
...
function Brainstorm.auto_reroll()
  ...
  pcall(function()
    immolate.immolate_set_cuda_enabled(use_cuda)
  end)
...
-- Local attention text helper for Brainstorm status messages.
