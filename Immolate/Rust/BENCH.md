...
C++ oracle DLL and for comparing the Rust CPU engine against the CUDA fast path.
...
For CUDA work, compare against the Rust CPU path with identical fixture, budget,
thread count, repeat count, and warmup count. The in-game switch is the
Brainstorm settings toggle. For command-line profiling, temporarily build with
`BRAINSTORM_SKIP_CUDA_BUILD=1` for an explicit CPU-only baseline.
...
Run the native Rust helper for a supported CUDA fixture:

```bash
cargo run --manifest-path Immolate/Rust/Cargo.toml --release --bin brainstorm_bench -- \
  --case ux-tag-voucher-pack --budget 1000000 --threads 1 --repeat 5 --warmup 1
```

For a direct CPU-only comparison from the command line, use a build that embeds
empty PTX:

```bash
BRAINSTORM_SKIP_CUDA_BUILD=1 cargo run --manifest-path Immolate/Rust/Cargo.toml --release --bin brainstorm_bench -- \
  --case ux-tag-voucher-pack --budget 1000000 --threads 1 --repeat 5 --warmup 1
```
...
`mise run doctor` to check the system dependencies. Benchmarking still needs the
same runtime tools used by the Rust rewrite gate:
...
- CUDA Toolkit with `nvcc` and a compatible host compiler for GPU-enabled Rust
  builds. On Ubuntu/WSL, `nvidia-cuda-toolkit` plus `gcc-12` matches the default
  `CUDAHOSTCXX=gcc-12`. This branch defaults to `BRAINSTORM_CUDA_ARCH=sm_89` for
  RTX 4090. Set `BRAINSTORM_SKIP_CUDA_BUILD=1` only for explicit CPU-only builds.
...
- `BRAINSTORM_CUDA_LAUNCH_SEEDS=2000000`
