...
use immolate::engine::cuda::debug_seed_cuda;
use immolate::filters::FilterConfig;
use immolate::instance::Instance;
use immolate::item::Item;
...
fn main() {
    ...
    if std::env::var_os("BRAINSTORM_DEBUG_CUDA").is_some() {
        for id in [0, 1, 2, 3, 35, 36, 37, 1_000, 1_000_000] {
            let mut inst = Instance::new(Seed::from_id(id));
            inst.init_locks(1, false, false);
            inst.set_deck(Item::Red_Deck);
            let small = inst.next_tag(1) as usize;
            let big = inst.next_tag(1) as usize;
            let voucher = inst.next_voucher(1) as usize;
            let _first_pack = inst.next_pack(1) as usize;
            let pack = inst.next_pack(1) as usize;
            if let Some(gpu) = debug_seed_cuda(id) {
                println!(
                    "cuda_debug_{id}=hash:{:.17},tag:{}/{},voucher:{},pack:{} cpu_tag:{}/{},cpu_voucher:{},cpu_pack:{}",
                    f64::from_bits(gpu[0]),
                    gpu[1],
                    gpu[2],
                    gpu[3],
                ...
                println!("cuda_debug_{id}=unavailable");
            }
        }
    }
