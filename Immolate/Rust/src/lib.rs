...
mod tests {
    ...
    #[test]
    fn cuda_fast_path_matches_cpp_oracle_when_available() {
        ...
        for (name, cfg, expected) in cases {
            ...
            let cuda_result = search_cuda("", &compiled, 100_000);
            assert_ne!(
                cuda_result,
                CudaSearch::Unsupported,
                "CUDA unexpectedly unsupported for {name}",
            );
            match cuda_result {
                CudaSearch::Unavailable => return,
                CudaSearch::Unsupported => return,
                CudaSearch::Complete(result) => {
                    saw_cuda = true;
                    assert_eq!(result.as_deref(), expected, "CUDA mismatch for {name}");
                },
            }
        }
        assert!(saw_cuda);
