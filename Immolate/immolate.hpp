...
// C API surface for the Brainstorm native DLL
extern "C" {
// Returns a heap-allocated string (free with free_result).
IMMOLATE_API const char* brainstorm_search(const char* seed_start,
                                           ...
                                           long long num_seeds,
                                           int threads);
// Sets the log path used by the native DLL (optional).
IMMOLATE_API void immolate_set_log_path(const char* path);
// Frees strings returned by brainstorm_search.
