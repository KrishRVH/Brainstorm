...
if [ "$CONTINUE" = true ]; then
    ...
    if [ -f "ImmolateCPU.dll" ]; then
        DLL_SIZE=$(stat -c%s "ImmolateCPU.dll" 2>/dev/null || stat -f%z "ImmolateCPU.dll" 2>/dev/null)
        if [ "$DLL_SIZE" -gt 2000000 ] && [ "$DLL_SIZE" -lt 10000000 ]; then
            echo -e "${GREEN}✓ CPU DLL size OK: $(($DLL_SIZE / 1048576))MB${NC}"
        else
            echo -e "${RED}✗ CPU DLL size suspicious: $(($DLL_SIZE / 1048576))MB${NC}"
        fi
    fi
